...
inline static size_t ksetintmap_hashing(KBitSet* key) {
  if (!key) return 0;
...
static void ksetintmap_rehash(KevSetIntMap* to, KevSetIntMap* from) {
...
static bool ksetintmap_expand(KevSetIntMap* map) {
  KevSetIntMap new_map;
  if (k_unlikely(!ksetintmap_init(&new_map, map->capacity << 1)))
    return false;
  ksetintmap_rehash(&new_map, map);
  *map = new_map;
  return true;
}

static void ksetintmap_bucket_free(KevSetIntMapNode* bucket) {
  while (bucket) {
...
void ksetintmap_destroy(KevSetIntMap* map) {
  ...
  for (size_t i = 0; i < capacity; ++i)
    ksetintmap_bucket_free(array[i]);
  free(array);
  map->array = NULL;
  map->capacity = 0;
  map->size = 0;
